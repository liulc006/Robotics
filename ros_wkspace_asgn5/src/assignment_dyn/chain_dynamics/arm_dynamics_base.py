...
class ArmDynamicsBase:
    ...
    def compute_pos(self, pos_0, theta):
        ...
        for i in range(1, self.num_links):
            pos[2 * i:2 * (i + 1)] = pos[2 * (i - 1):2 * i] + np.dot(rot(theta[i - 1]), self.link_lengths[i - 1] * xaxis())
        return pos

    def compute_pos_com(self, pos, theta):
        pos_com = np.zeros((2 * self.num_links, 1))
        for i in range(0, self.num_links):
            pos_com[2 * i:2 * (i + 1)] = pos[2 * i:2 * (i + 1)] + np.dot(rot(theta[i]), 0.5 * self.link_lengths[0] * xaxis())
        return pos_com
